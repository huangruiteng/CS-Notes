...
* 整书笔记：《开源项目成功之道》（John Mertic，人民邮电出版社）→ [Software-开源项目成功之道.md](Software-开源项目成功之道.md)
...
#### Log4j / Log4Shell：被全球依赖、却没人买单的维护者

> 来源：[CVE-2021-44228（NVD）](https://nvd.nist.gov/vuln/detail/CVE-2021-44228)、[Apache Logging Services 获 Sovereign Tech Fund 资助公告](https://logging.apache.org/blog/2023/12/14/announcing-support-from-the-stf.html)、[开源中国：Log4j2 维护者吐槽没工资还要挨骂](https://www.oschina.net/news/173781/open-source-authors-and-companies)、[CSO Online 2024：三年后仍有 13% 下载在拉易受攻击版本](https://www.csoonline.com/article/3560646/malicious-open-source-software-packages-have-exploded-in-2024.html)。整理时间 2026-08-16。

**事件**：2021-12-09 前后，Log4j 2 的 JNDI lookup 远程代码执行漏洞（Log4Shell，CVSS 10.0）被公开。Log4j 是 Java 世界事实标准的日志库，被数百万应用依赖，从苹果到 Minecraft 都在用；漏洞曝光几小时内公开 exploit 就已出现，各公司连夜排查。

**维护者受到的冲击**：

- **全球关键基础设施靠几个志愿者**：项目主要由 Ralph Goers 等志愿者维护，事发时 Ralph 在 GitHub 上只有 3 个赞助者，几乎没有财务支持；漏洞公开后他和小团队连续多日连轴转出补丁，同时承受大量指责。
- **“白嫖 + 无限责任 + 挨骂”**：无数公司依赖它却没有人付钱；出了漏洞，维护者既没报酬又要负无限责任，还要面对蜂拥而至的 issue、重复报告和网络攻击。
- **补丁本身也是高压迭代**：2.15.0（默认关闭 JNDI）→ 2.16.0（修 CVE-2021-45046）→ 2.17.0（修 CVE-2021-45105 DoS）→ 2.17.1，连续补丁说明第一版修复不完整；任何不完美都会被放大成“开源不靠谱”的指责。
- **漏洞长尾远未结束**：2024 年调查显示 Maven Central 上仍有约 13% 的 log4j 下载在拉取易受攻击版本——漏洞会淡出新闻，但风险不会自动消失。

**后果与治理改进**：

- 2023-12，德国 Sovereign Tech Fund 开始资助三名维护者（Christian Grobmeier、Piotr Karwasz、Volkan Yazıcı），其中两人为此辞去全职工作；
- 资助落地后项目建了 CI 化发布流水线、现代化代码与依赖、开始为发布产物生成 SBOM 和漏洞披露报告（VDR）、大幅改进文档。
- 行业层面的反思：关键开源基础设施需要被当作公共基础设施付费维护；安全不只是“发现漏洞”，还包括可追溯的供应链（SBOM）、升级通道和可持续的维护者队伍。
...
![SemiAnalysis subscription margin](./非技术知识/semianalysis-ai-subscription-margin-utilization.png)

![SemiAnalysis max possible spend](./非技术知识/semianalysis-ai-subscription-max-spend.png)
