...
* ItemCF
  ...
  * 用户对一个新物品的兴趣度，等于这个新物品与他所有历史喜好物品(`U2I`)的相似度的加权总和。因此实现时先一跳拉最近行为，再二跳 merge
...
#### Online learning、off-policy 与反馈循环

推荐系统的流式 online learning 很接近 near-on-policy，但通常不是严格 on-policy。
...
当前 policy 产生曝光
-> 当前 policy 立即用这些反馈更新
-> 更新后旧数据不再被当成当前策略数据反复使用
```

工业推荐即使链路延迟很低，也仍然会有几个偏差来源：

- **policy staleness**：样本来自几分钟前、几小时前或昨天的 policy，而训练更新的是新 policy。
- **exposure selection bias**：模型只能学习旧策略曝光过的 item；没曝光的候选没有 label。
- **delayed feedback**：转化、留存、复访等 label 天然滞后，低延迟点击反馈不等于长期 reward 实时。
- **supervised objective**：多数排序模型仍在拟合 logged label，而不是让当前 policy rollout 后直接优化长期回报。

所以流式训练降低的是 staleness，不自动消除 off-policy。它是更“近”的 logged supervised learning，而不是完整 RL。

更系统的 logged policy、IPS / importance sampling、clipping、doubly robust 与 off-policy learning 公式，见 `Reinforcement-Learning.md` 的 `Counterfactual evaluation / Off-policy evaluation`。这套理论更适合放在 RL / OPE 基础里；推荐系统里只保留应用视角：曝光日志来自旧策略，因此普通离线 AUC 不等价于新策略真实收益。

这个视角可以重新理解推荐系统的几个老问题：

- **兴趣遗忘**：如果某批用户 / item / 兴趣模式之后不再出现在训练样本里，共享 dense 参数可能逐步不再服务这些模式。一个可验证实验是：在某天后移除一批用户样本，排除兴趣自然迁移后跟踪这批用户 AUC 是否持续下降。
- **反馈循环**：上一版策略决定曝光，曝光日志训练下一版策略；模型可能越来越擅长当前局部最优，却越来越难发现未曝光、冷启动或长尾模式。
- **负迁移**：多任务 / 多场景共享 dense 参数时，不一定只是梯度夹角冲突，也可能是不同任务都在用 supervised label 覆盖共享主方向，导致旧模式被覆盖。
- **稀疏隔离**：传统推荐的大规模 embedding 表在一定程度上隔离了用户、item、场景间干扰；随着 dense scaling、Transformer 化和 embedding 压缩增强，这种天然隔离会变弱，LLM 式遗忘 / interference 可能更显性。

因此推荐系统里大量探索、冷启动、打散、规则兜底和策略回退，也可以看成在弥补 supervised feedback loop 的副作用：模型负责拟合已发生反馈，工程策略负责打开没有被当前模型充分探索的未来数据分布。

参考：[从“RL比SFT更不容易遗忘”到“反观推荐系统缺陷”青稞转载](https://qingkeai.online/archives/D9j6JtwN) / [知乎原文](https://zhuanlan.zhihu.com/p/1976336943878005134)。
...
### 向量存储冷热分层：Vector Bucket / S3 Vectors / TOS Vectors

> 参考：[Vector Bucket：云原生向量存储新范式](https://mp.weixin.qq.com/s/468ign3wZahV7eTK4as2FQ)

* 背景：AI 搜推问、RAG、Agent memory、多模态检索会产生大量向量。传统向量数据库通常是存算一体：索引、向量、元数据和查询计算绑定在实例内，追求低延迟时往往要把索引常驻内存或 SSD，成本与 RAM / SSD / 实例规格强绑定。
* 新问题：很多 ToC / Agent / SaaS 场景是“小规模、海量多用户”：
  * 用户数多，但单个用户的向量规模不一定大。
  * 大量历史向量查询频率低、更新频率低。
  * 延迟要求不一定像主链路推荐/广告召回那样苛刻。
  * 如果全部放在内存型向量数据库、MongoDB、OpenSearch 或实例型存储里，长期持有成本可能大于收益。
* Vector Bucket / S3 Vectors / TOS Vectors 的核心定位：把对象存储从“文件 KV”扩展成原生支持向量语义的低成本容量层。
  * 向量持久化放在 S3/TOS 这类对象存储上，获得低成本、高可靠、弹性容量。
  * 计算层通过索引、缓存、导入/导出与热数据系统协同，按需把高频或高优先级数据迁移到 OpenSearch / VikingDB / 云搜索等低延迟系统。
  * 用查询速度、召回率、PUT/Query 按量成本换取海量向量的长期存储成本下降。
* 和“PS 的 SSD 冷层”类比：
  * 类似点：都承接海量、低频、较冷、但又不能丢的 user / item embedding；目标是降低存储成本，而不是把所有 embedding 都放在昂贵内存里。
  * 差异点：Parameter Server 更偏训练/在线学习里的稀疏参数服务，通常要处理 optimizer state、参数更新、一致性和训练吞吐；Vector Bucket 更偏向量检索/语义存储的容量层，核心是持久化、ANN/近似查询、冷热迁移和按需检索。
  * 更准确的说法：它不是 PS 本身，而是 AI 搜推问系统里“向量索引体系的低成本冷/温层”，有点像给低频 embedding 准备的对象存储版 SSD/冷库。
* 适合场景：
  * 长尾用户画像、历史交互、历史内容 embedding。
  * Agent memory / RAG 中低频访问的长期记忆。
  * 多租户 SaaS 中每个 tenant 数据不大、但 tenant 数量很多的向量集合。
  * AI 搜索/推荐/问答中可接受较高延迟或分层召回的冷数据。
* 不适合单独承担：
  * 主链路高 QPS、低 p99 的热召回。
  * 高频写入/频繁更新 embedding 的实时训练链路。
  * 强依赖最新参数、一致性和 optimizer state 的 PS 语义。
* 关键 trade-off：
  * 存储成本下降，但 PUT / query 可能按请求计费；高频写入或高 QPS 查询下，计算/请求成本可能超过存储成本。
  * 冷数据长期保存更便宜，但热路径通常仍需 OpenSearch / VikingDB / Qdrant / 自研 ANN 服务。
  * 产品设计上，Vector Bucket 更像“向量存储层”，而不是完整替代向量数据库。
...
* **核心方案 (DMF Framework)**:
  ...
  *   **CMM (Complementary Modality Modeling)**:
      ...
      *   insights
          *   **低活用户**：行为数据稀疏，难以准确建模个性化偏好。此时，基于全局语义分布的“泛化能力”更为重要。
      
          *   **高活用户**：行为丰富，适合精细化建模其独特兴趣轨迹，强调“个性化”。
...
### IntTravel [Alibaba AMAP]: 多任务生成式推荐与POI场景

> **论文**: IntTravel: Large-Scale Dataset and Decoder-Only Generative Framework for Multi-Task Travel Recommendation
> 
> **链接**: https://arxiv.org/html/2602.11664v1, https://github.com/AMAP-ML/IntTravel
> 
> **核心贡献**: 
> - 首个大规模多任务旅行推荐数据集（41亿交互）
> - 多任务 decoder-only 生成式框架
> - 已在高德地图上线，CTR提升1.09%

#### 1. 背景与问题

* **现有数据集局限**：
  - 仅关注 Next POI 推荐（Where），忽略 When（出发时间）、How（出行方式）、Via（途中意图）
  - 数据规模小，无法准确评估生成式模型
* **生成式范式转变**：推荐模型从传统深度学习转向生成式范式，但传统多任务框架与生成式架构不兼容
...
* **四大核心任务**：
  1. **When (departure time)**: 预测用户出发时间
  2. **Where (destination)**: 推荐目的地 POI
  3. **How (travel mode)**: 预测出行方式（驾车、步行、公共交通等）
  4. **Via (on-the-way intention)**: 预测途中可能访问的 POI

* **序列构建**：
  - 三种 Token 类型：
    - **S (Scenario tokens)**: 时空上下文
    - **I (Item tokens)**: 交互 POIs 序列
    - **F (Feedback tokens)**: 交互类型（点击、下单等）
  - 多任务共享同一输入序列，各任务有独立标签序列
  - 标签附着策略：
    - When/How/Where 任务标签附着于 S tokens
    - Via 任务标签附着于 I tokens

#### 3. 多任务生成式框架

IntTravel 包含三个核心模块：

##### 3.1 Task-Guided Information Persistence (TIP)

* **核心思想**：将 Hyper Connection (HC) 扩展为多任务版本，嵌入 HSTU 作为 decoder
* **Hyper Connection 机制**：
  $$X_{l+1} = H^{\mathrm{res}}_{l} X_{l} + (H^{\mathrm{post}}_{l})^{\top} F(H^{\mathrm{pre}}_{l} X_{l}, W_{l})$$
* **任务门控机制**：
  - 为每个任务 k 生成任务特定参数 $J^{\mathrm{pre}}_{l,k}$ 和 $J^{\mathrm{res}}_{l,k}$
  - 通过元素级乘积：$J^{\mathrm{pre}}_{l,k} = H^{\mathrm{pre}}_{l} \odot \gamma_{l,k}$
  - 保持计算复杂度为 $\mathcal{O}(N^{2}C)$

##### 3.2 Task-Specific Selective Gating (TSG)

* **目标**：为每个任务选择有用的信息
* **三步流程**：
  1. **聚合**：将 TIP 的多流输出沿流维度聚合：$z_{l} = \text{Agg}(X_{l})$
  2. **任务门控**：从任务 embedding $e_{k}$ 生成层特定门控标量 $s_{l,k}$
  3. **选择性过滤**：$z_{l,k} = s_{l,k} \odot z_{l}$，再聚合所有层得到 $z_{k}$

##### 3.3 Task-Aware Scenario Factorization (TSF)

* **核心设计**：共享专家 + 任务私有专家
  - 共享专家 $\mathcal{E}^{s}$：通用知识
  - 私有专家 $\mathcal{E}^{p}_{k}$：任务特定知识
* **动态参数生成**：
  - 任务上下文：$R_{k} = \text{concat}(e_{k}, p)$，其中 $p$ 是用户画像
  - 专家权重：$\beta_{e} = 2 * \sigma(\text{MLP}(R_{k}))$
  - 最终参数：$W_{k} = \sum_{e \in \mathcal{E}^{s} \cup \mathcal{E}^{p}_{k}} \beta_{e} \tilde{W}_{e}$，$b_{k}$ 同理
* **输出**：$\hat{y}_{k} = W_{k} \tilde{z}_{k} + b_{k}$

#### 4. 实验结果

* **IntTravel 数据集**：
  - 在所有四个任务上均取得 SOTA
  - TSF 模块贡献最大，动态生成参数对任务-用户场景至关重要
* **Tenrec-QK-video 数据集**（非旅行领域）：
  - 在四个预测任务（点击、点赞、分享、关注）上均优于基线
  - 证明了架构的泛化能力
* **Scaling Laws**：
  - 模型深度从 1 层到 80 层，性能持续提升
  - 无性能下降迹象，展现了优秀的扩展性
* **在线 A/B Test**：
  - 在高德地图上线
  - CTR 提升 1.09%
...
* **多任务生成式融合**：传统多任务框架与生成式架构不兼容，需设计专门的信息保留、选择、因子化机制
* **标签附着策略**：防止数据泄露的关键，需根据任务预测目标将标签附着在正确的 token 类型上
* **共享与私有专家**：平衡任务协作与任务特异性的有效方式
* **Scaling Law 验证**：生成式推荐模型同样遵循 scaling law，深度增加性能持续提升
...
* **RegAUC (Regression AUC)**:
  * 背景：普通 AUC 要求二分类标签，但推荐系统里常见目标是连续值，例如观看时长、停留时长、收益、满意度分、互动强度。此时如果线上排序目标更关心“谁应该排在谁前面”，而不是绝对数值误差，就可以用 RegAUC 看连续标签下的排序一致性。它本质上是 regression 版本的 pairwise AUC / concordance index。
  * 直觉：随机抽两个样本 $i, j$，如果真实标签 $y_i > y_j$，模型预测分 $\hat{y}_i$ 也应该大于 $\hat{y}_j$。RegAUC 衡量这种 pairwise order 被排对的概率。
  * 常见定义：先定义可比较 pair。为了避免双向重复，通常只取有方向的 pair：
    $$
    \mathcal{P}_{\epsilon}=\{(i,j)\mid y_i > y_j + \epsilon\}
    $$
    $$
    RegAUC =
    \frac{1}{|\mathcal{P}_{\epsilon}|}
    \sum_{(i,j)\in\mathcal{P}_{\epsilon}}
    \left[
      \mathbb{I}(\hat{y}_i > \hat{y}_j + \delta)
      + 0.5 \cdot \mathbb{I}(|\hat{y}_i-\hat{y}_j| \le \delta)
    \right]
    $$
    其中 $\epsilon$ 是 label margin，$\delta$ 是预测分 tie tolerance。没有特殊噪声控制时可取 $\epsilon=0$；如果观看时长、GMV、停留时长等 label 噪声大，可以只统计 $|y_i-y_j|>\epsilon$ 的 pair。
  * **label 相等的 pair 是否从分母剔除**：默认应该剔除。
    - 原因一：当 $y_i=y_j$ 时，真实标签没有提供谁应该排在前面的监督信号；把它放进分母，相当于把“不可比较 pair”当成“模型必须处理的 pair”。
    - 原因二：二分类 AUC 的 pairwise 解释本来只比较正负样本，正正 / 负负 pair 不进分母。RegAUC 作为连续标签推广，也应只比较 label 可区分的 pair。
    - 原因三：concordance index 的常见实现也把 tied outcome / tied response pair 视为不可比较并忽略；预测分 tie 才通常按 $0.5$ 计入 concordant count。
    - 工程后果：如果把 label 相等 pair 放进分母但不给分，label 粒度越粗或 0 值越多，RegAUC 会被大量无监督意义的 pair 压低；如果给这些 pair 记 $0.5$，又会把指标往 0.5 稀释，掩盖真正有顺序差异的 pair。
  * 什么时候可以不剔除：如果业务明确要求“相同 label 的样本必须预测成相同分数”，那已经不是纯 RegAUC，而是把 equality preservation / calibration 也揉进来了。更干净的做法是拆成两个指标：
    - RegAUC：只看 $y_i \neq y_j$ 或 $|y_i-y_j|>\epsilon$ 的排序一致性；
    - tie / calibration 指标：单独看同 label 样本的预测分方差、分桶 bias、MSE / MAE。
  * 没有可比较 pair 怎么办：如果一个 user / query / request group 内所有 label 都相等，组内 RegAUC 应记为 undefined / NaN，并在 GAUC 聚合时跳过，同时报告 valid group coverage；不要强行记成 0.5 或 0。
  * 与 AUC 的关系：当 $y$ 退化成 0/1 标签时，RegAUC 就退化为普通 AUC 的 pairwise 解释；连续标签下，它更接近 concordance index / pairwise ranking accuracy。
  * 与 MSE / MAE 的差别：MSE/MAE 衡量数值拟合准不准；RegAUC 衡量相对顺序排得对不对。一个模型可能 RegAUC 高但 calibration 差，也可能 MSE 好但排序顺序不够好。
  * 适用场景：
    - 连续 label 训练，但 serving 最终按 score 排序；
    - watch time / dwell time / value 等长尾分布目标；
    - 希望离线指标更贴近排序收益，而不是被少量极大值主导。
  * 注意点：
    - $O(n^2)$ 全量 pair 成本高，工业实现通常采样 pair 或按分桶近似。
    - 要按 query / user / request group 内计算更有意义，否则容易混入用户活跃度、流量分布、item 热度等 group bias。
    - 它不关心绝对值是否可校准，不能替代 MSE、MAE、bias、分桶校准和线上业务指标。
  * 参考：[scikit-survival `concordance_index_censored`](https://scikit-survival.readthedocs.io/en/latest/generated/sksurv.metrics.concordance_index_censored.html) 对 concordance index 的定义是“comparable pairs 中预测和结果一致的比例”，并明确同一事件时间的 pair 不可比较、预测风险打平按 0.5 处理；R `survival` 的 [Concordance](https://cran.r-universe.dev/survival/doc/concordance.pdf) 文档也说明 tied response values 的 pair 会被忽略。这两点支持 RegAUC 默认剔除 label 相等 pair 的口径。
...
### Agent Memory Routing as Recommendation

> 来源：[How Memory Management Impacts LLM Agents](https://arxiv.org/abs/2505.16067)，用户 2026-05-03 精读后的抽象。
> Canonical 领域入口见：`Notes/AI-Applied-Algorithms.md` 的 `Agent Memory Recommendation / Routing：把记忆系统建模成推荐系统`。本节保留推荐系统视角的交叉索引。

Agent memory routing 可以自然改写成推荐问题：`worker + task + domain + runtime state` 是 user/context，`experience / procedure memory / trajectory fragment` 是 item，memory 被注入上下文是 exposure，agent 遵循它并改变行动相当于 click/convert，最终 reward 是 paired outcome delta。

这个抽象的关键不是“memory 越多越差”，而是：memory_size 提供 coverage，memory_noise / misalignment 带来负迁移。候选池变大后，系统需要的是 item quality、precondition、post-exposure utility 和 lifecycle，而不是简单 top-k 相似度。
...
- `source_quality_gate` 类似 item admission / 内容质量审核：源轨迹不可靠就不进 durable memory。
- `applicability_gate` 类似强过滤：domain、权限、状态、前提不匹配时，不能靠 boost/bury 修正。
- `post_exposure_utility` 类似曝光后反馈：memory 被注入后是否真的提升 outcome，决定 promote、keep、rewrite、bury、delete。
- `history-based deletion` 类似 item 长期反馈治理：不是按频次或时间删，而是按多次曝光后的平均 future utility 处理。

这给 Agent Harness 的建模切口是：先导出 `(task, candidate_memory)` 级别的离线数据，再做 rule / logistic / GBDT / two-tower / sequence model 的逐级 baseline；不要一开始就跳到 LLM fine-tuning 或 RL。
