- 背景：10天体验 + 381个commit + 9 亿 Token，和父母外出旅游期间，路上和 OpenClaw 交互，做了一个 Trivial 的 Backlog 管理玩具项目
1. 观点先行
...
本质是易用性：1）Claude Code 急忙推出 Web Manager 能力，一定程度体现了过往产品对易用性的忽视；2）火山 AI 知识管理产品的浏览器插件（一些产品思考 by @苏奇 ），也是建设易用性的一种思路。
    - 脑：LLM 的推理规划能力。
...
- AI 赋能个体的工作、生活、学习效率是至关重要的，因此 2.17 - 2.26 的 10 天体验中，我做了一个基础尝试：基于 Backlog（待办事项）的 Proactive Agent，帮助我进行笔记管理和 todos 管理
代码不复杂，一个玩具项目，主要想分享：
1）构建项目过程中和 AI 的协作经验；
2）Vibe Coding 优化工作和学习效率的思路；
3）统一管理 & AI Coding 公司内外部 tasks 的最佳实践探索。
- 模式的核心价值：
...
dev 环境开了 web manager 的公网接口，想了解更多的同学可找我
...
- 技能集成数量：20+ 个 Skills 集成
  - -> 复利效应
- Token 消耗：8.7 亿 Token（有 prefix cache）
  - 4.7*3.2*100 + 4*0.64*100 = 1760 元
  - 调用次数 9600 次
  - 改用 Coding Plan Pro，每月 9w 次调用额度，一个月 200 元基本勉强能 cover
[图片]
...
3. 个性化 Agent 讨论
OpenClaw 实现个性化 Agent，本质是基于 PE 和 Workflow
...
Appendix: 内部 OpenClaw 实践资料
https://www.volcengine.com/docs/6396/2189942?lang=zh
ClawdBot(Moltbot/OpenClaw) - 实操手册
字节内部OpenClaw安装及最佳实践探索